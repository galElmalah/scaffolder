...
const { MissingKeyValuePairs, MissingTransformerImplementation, } = require("../../Errors");
    ...
    describe("injector with applyTranformers", () => {
        ...
        `;
            ...
            const globalCtx = {
                templateName: "what",
                templateRoot: "yay",
                parametersValues: Object.assign({}, keys),
            ...
            const keysInjector = injector(keys, { transformers, functions }, globalCtx);
            const result = keysInjector(testTemplate, localCtx);
            expect(transformers.toLowerCase).toHaveBeenCalledWith("YEAH", Object.assign(Object.assign({}, globalCtx), localCtx));
            expect(transformers.repeat).toHaveBeenCalledWith("yeah", Object.assign(Object.assign({}, globalCtx), localCtx));
            expect(functions.date).toHaveBeenCalledWith(Object.assign(Object.assign({}, globalCtx), localCtx));
            expect(result).toBe(`
        ${keys.key1}Whats${keys.key3}
        const handleError = ${keys.key1.toLowerCase() + keys.key1.toLowerCase()} => {
          if (${keys.key1}.getDisplayErrorMessage) {
            console.log(${keys.key1}.getDisplayErrorMessage());
          } else {
            console.error(${keys.key2});
        ...
        `;
            ...
            expect(() => keysInjector(testTemplate)).toThrowError(MissingTransformerImplementation);
        });
    });
});
