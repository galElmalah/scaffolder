const fs = require('fs');
const { NoMatchingTemplate, MissingKeyValuePairs, MissingFunctionImplementation, } = require('../../Errors');
...
const getKeyAndTransformers = (initialKey) => extractKey(initialKey)
    .split('|')
    .map((_) => _.trim());
const replaceKeyWithValue = (keyValuePairs, transformersMap, functionsMap, ctx) => (match) => {
...
const getConfigPath = (path) => path.split('/').slice(0, -1).join('/') + '/scaffolder.config.js';
const templateReader = (commands) => (cmd) => {
    ...
    if (fs.existsSync(getConfigPath(commands[cmd]))) {
        ...
        config = Object.assign(Object.assign({}, defaultConfig()), require(getConfigPath(commands[cmd])));
    }
    return {
        config,
...
const templateTransformer = (templateDescriptor, injector, globalCtx) => {
    ...
    const transformFile = (descriptor) => ({
        name: injector(descriptor.name, createLocalCtx(Object.assign(Object.assign({}, descriptor), { type: TYPES.FILE_NAME }))),
        content: injector(descriptor.content, createLocalCtx(Object.assign(Object.assign({}, descriptor), { type: TYPES.FILE_CONTENT }))),
    });
 ...
 */
const injector = (keyValuePairs, { transformers = {}, functions = {} } = {}, globalCtx) => (text, localCtx) => {
    const ctx = Object.assign(Object.assign({}, globalCtx), localCtx);
    const keyPattern = new RegExp(keyPatternString, 'g');
    const replacer = replaceKeyWithValue(keyValuePairs, transformers, functions, ctx);
