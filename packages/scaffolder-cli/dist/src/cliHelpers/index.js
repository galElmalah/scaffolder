...
const generateKeyValues = (cmd) => cmd.parent.rawArgs
    .filter((arg) => arg.includes('='))
    .map((keyValuePair) => keyValuePair.split('='))
    .reduce((accm, [key, value]) => (Object.assign(Object.assign({}, accm), { [key.trim()]: value.trim() })), {});
const showSuccessMessage = (command, createdAtPath) => {
    const message = `${multiColors('Hooray!!')}\nSuccessfuly created the ${boldGreen(command)} template at ${path(createdAtPath)}
    `;
    console.log(message);
};
const getLongestCommand = (commands) => {
    return Math.max(...Object.keys(commands)
        .filter((c) => c[0] !== '.')
        .map((c) => c.length));
};
