...
const writeFilePromise = (path, content) => new Promise((resolve, reject) => {
...
class TemplatesBuilder {
            ...
            return Promise.all(folderDescriptor.content.map((descriptor) => {
                try {
                    if (descriptor.type === TYPES.FOLDER) {
                        return this.createTemplateFolder(descriptor, join(root, folderDescriptor.name));
                    }
                    return writeFilePromise(join(root, folderDescriptor.name, descriptor.name), descriptor.content).catch((e) => console.log('Error::createTemplateFolder::', e));
                }
                catch (e) {
    ...
    build() {
        ...
        this.templates.forEach((template) => {
            ...
            if (template.type) {
                promises.push(this.createTemplateFolder(template, join(this.entryPoint, this.pathPrefix, this.folder)));
                return;
            }
