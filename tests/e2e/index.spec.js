...
const execOnTestDir = (cmd) => execSync(`node ../../cli.js ${cmd}`, { stdio: 'inherit', cwd: __dirname })
const cleanUp = folder => rimraf.sync(`${__dirname}/${folder}`);
const isFolderExists = folder => existsSync(`${__dirname}/${folder}`)
const isFileContainsText = (path, content) => readFileSync(path, 'utf-8').includes(content)
